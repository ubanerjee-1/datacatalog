...
_SETUP_GOLD_DDL: dict[str, str] = {
    ...
    # Customer-editable affiliates (operating subsidiaries). Generated by
    # the company-research wizard via _ai_query_generate_affiliates(); rows
    # with is_user_edited=true survive AI re-runs and are managed via the
    # Edit Center. The legacy CSV seed path (build_value_model.py
    # seed_affiliates stage) was retired in favor of LLM generation so the
    # app onboards cleanly for any company, not just BHE.
...
# Strict JSON Schema for affiliate generation. Single ai_query call per
# company (no fan-out) returns the full subsidiary list.
_AFFILIATES_RESPONSE_SCHEMA: str = json.dumps({
    "type": "json_schema",
    "json_schema": {
        "name": "affiliates_response",
        "schema": {
            "type": "object",
            "properties": {
                "affiliates": {
...
def _ai_query_generate_affiliates(company: str) -> list[dict]:
    """Generate the operating-subsidiary list for `company` via a single ai_query call.

    Single-row ai_query (no VALUES fan-out): the LLM's training data has
    good coverage of major-company subsidiary structure, so we just ask
    for the full list in one shot. Output shape matches `bhe_gold.affiliates`
    columns so the caller can MERGE directly.

    Returns: list of dicts with keys affiliate_name, affiliate_code,
    business_type, region, description. Empty list on parse failure (caller
    is responsible for surfacing that).
    """
    company_esc = _sql_escape(company)
    schema_esc = _sql_escape(_AFFILIATES_RESPONSE_SCHEMA)

    prompt = (
        f"You are a corporate research analyst. Identify the operating "
        f"affiliates / subsidiaries / business units of {company_esc}. "
        f"Return all material operating entities, including utility "
        f"subsidiaries, energy / commodity businesses, infrastructure "
        f"businesses, and corporate / holding entities. "
        f"For each affiliate provide: "
        f"affiliate_name (canonical legal or commonly-used brand name); "
        f"affiliate_code (short ticker-style code, 2-6 chars, uppercase, "
        f"may be empty for entities without a common short code); "
        f"business_type (one of: electric_utility, electric_gas_utility, "
        f"gas_utility, renewables_developer, natural_gas_pipeline, "
        f"electric_transmission, electric_distribution, energy_trading, "
        f"corporate, other); "
        f"region (geographic footprint, e.g. 'Pacific Northwest, US' or "
        f"'UK Northeast'); "
        f"description (1-2 sentence summary of what the affiliate does)."
    )
    ...
    if not rows:
        return []
    raw = rows[0].get("result") or ""
    if not raw:
        return []
    try:
        parsed = json.loads(raw)
        affs = parsed.get("affiliates", []) if isinstance(parsed, dict) else []
        if isinstance(affs, str):
            affs = json.loads(affs)
        if not isinstance(affs, list):
            return []
        return affs
    except Exception as e:
        logger.warning(f"_ai_query_generate_affiliates parse failed: {e}")
        return []
...
def _run_company_research(
    ...
    def _company_count(table: str) -> int:
        """Count rows in a research-output table.

        Most research tables live in `bhe_silver` and carry a `company_name`
        column (single-tenant per deploy, but historically scoped). The
        `affiliates` table is the exception: it lives in `bhe_gold` and has
        no `company_name` column, so we count all rows.
        """
        try:
            if table == "affiliates":
                rows = execute_query(
                    f"SELECT count(*) AS n FROM {fqn(gold, table)}"
                )
            else:
                rows = execute_query(
    ...
    try:
        ...
        # ------------------------------------------------------------------
        # Company research generates company profile + departments + affiliates.
        ...
        # Affiliates stay grounded in LLM training data (subsidiary structure
        # of public companies is well-known) so they fit the same single-call
        # pattern as profile + departments. Affiliates lives in `bhe_gold`
        # while profile + departments live in `bhe_silver`.
        step_to_table = {
        ...
        # ---- Step 3: Affiliates via single ai_query() call ----
        # Generates the company's operating subsidiaries / business units
        # from the LLM's training data (well-grounded for major companies)
        # and MERGEs into bhe_gold.affiliates. Manual edits with
        ...
        if _should_run("affiliates", "affiliates"):
            ...
            try:
                ...
                _log(f"Affiliates received: {len(affiliates)}")
            except Exception as e:
                _log(f"Affiliate generation FAILED: {e}")
                affiliates = []

            if affiliates:
                # Build a VALUES clause for MERGE. Skip rows missing
                # affiliate_name since that's the merge key.
                value_rows: list[str] = []
                for a in affiliates:
                    name = str(a.get("affiliate_name") or "").strip()
                    if not name:
                        continue
                    code = str(a.get("affiliate_code") or "").strip()
                    btype = str(a.get("business_type") or "").strip()
                    region = str(a.get("region") or "").strip()
                    desc = str(a.get("description") or "").strip()
                    value_rows.append(
                        f"('{_sql_escape(name)}', '{_sql_escape(code)}', "
                        f"'{_sql_escape(btype)}', '{_sql_escape(region)}', "
                        f"'{_sql_escape(desc)}', true)"
                    )

                if value_rows:
                    values_clause = ",\n              ".join(value_rows)
                    aff_tbl = fqn(gold, "affiliates")
                    # MERGE: update AI-managed columns when not user-edited;
                    # insert new affiliates with is_user_edited=false; never
                    # touch user-edited rows.
                    execute_query(f"""
                        MERGE INTO {aff_tbl} AS t
                        USING (
                            SELECT * FROM (VALUES
                              {values_clause}
                        ...
                        WHEN NOT MATCHED THEN
                            INSERT (affiliate_name, affiliate_code, business_type,
                                    region, description, is_active, is_user_edited,
                                    created_at, updated_at)
...
# Company research generates company profile + departments + affiliates.
# Use cases, sankey mappings, and entities have been removed because they
# need canonical source systems (loaded by CSV ingest) and user intent
# (chat-driven) to be grounded. Affiliates fit here because the LLM's
# training data has good coverage of major-company subsidiary structure
# (no canonical sources required).
ALL_RESEARCH_STEPS = ["profile", "departments", "affiliates"]

# Tables wiped by `_wipe_research_tables()` on a full reset. Only the
# silver tables — affiliates is preserved because it's MERGE-based and
# carries `is_user_edited=true` rows that must survive a research reset.
_RESEARCH_TABLES = [
    "company_profile",
    "departments",
]
...
def _wipe_research_tables() -> None:
    ...
    Wipes profile + departments only. Affiliates uses a MERGE-with-preserve
    pattern instead of DELETE — its is_user_edited rows must survive any
    company-research re-run. Use cases / sankey / entities live in their
    own flows (chat for UCs; future "build sankey" endpoint) and aren't
    touched here.
    """
    silver = get_silver_schema()
...
async def company_research_status() -> dict:
    ...
    # steps" that gate completion. Affiliates lives in gold (not silver)
    # and has no company_name column, so we count it via a separate path.
    ...
    # affiliates: gold-schema, no company filter
    try:
        gold = get_gold_schema()
        rows = execute_query(f"SELECT count(*) AS c FROM {fqn(gold, 'affiliates')}")
        counts["affiliates"] = int(rows[0].get("c", 0)) if rows else 0
    except Exception:
        counts["affiliates"] = 0
