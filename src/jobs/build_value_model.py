...
---
    ...
    --skip-llm                 skip stage 3 (seed only)
...
# Note: affiliates seeding moved to the company-research wizard
# (see _ai_query_generate_affiliates in src/app/.../router.py). This job
# no longer reads affiliates_seed.csv. Anyone needing custom affiliate
# entries uses the Edit Center, which sets is_user_edited=true so the
# wizard's MERGE never overwrites them.
...
def main() -> None:
    ...
    # ai_query (see _ai_query_generate_affiliates in router.py). The CSV
    # seed path was retired: anyone needing custom affiliates uses the
    # Edit Center (which sets is_user_edited=true and survives re-runs).
