...
def main() -> None:
    ...
    print("  - Walk through Company Setup wizard steps 1-7 (Database Bootstrap")
    print("    → Genie → Company Intelligence → Data Sources → Enrichment).")
    print("  - On /use-cases, run these one-time LLM-assisted setup actions in")
    print("    order so the Value & Readiness Sankey resolves through real")
    print("    BHE structure instead of LLM-guessed products:")
    print("      1. \"Map programs\"       — catalog prefix → program → affiliate")
    print("                                 (auto-fires populate-gold)")
    print("      2. \"Map data domains\"   — closed vocab of data needs +")
    print("                                 canonical → domain mappings")
    print("      3. \"Tag UCs with domains\"— backfills required_data_domains on")
    print("                                 existing use cases (idempotent)")
    print("    The 3 actions are required only on first deploy / fresh seed;")
    print("    subsequent UC generation tags domains automatically.")
